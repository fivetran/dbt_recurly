...
with spine as (
    ...
    {% endset %}
    {% set first_date_raw = run_query(first_date_query).columns[0][0] %}
    {% if first_date_raw is none %}
        {% set first_date = '2009-01-01' %}
    {% else %}
        {% set first_date = first_date_raw|string %}
    {% endif %}
    ...
    {% set last_date_raw = run_query(last_date_query).columns[0][0] %}
    {% set current_date_raw = run_query(current_date_query).columns[0][0] %}

    {% if last_date_raw is none %}
        {% set last_date = current_date_raw|string %}
    {% elif current_date_raw|string < last_date_raw|string %}
        {% set last_date = last_date_raw|string %}
    {% else %}
        {% set last_date = current_date_raw|string %}
    {% endif %}
