...
final as (
    ...
    select
        ...
        {% for f in fields %}
        coalesce({{ f }},
            first_value({{ f }}) over (partition by {{ f }}_partition {{ fivetran_utils.partition_by_source_relation(package_name='recurly', alias='account_partitions') }} order by date_day rows unbounded preceding)) as {{ f }}
