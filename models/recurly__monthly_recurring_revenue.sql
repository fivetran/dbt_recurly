...
mrr_by_account as (
    ...
    select
        ...
        row_number() over (partition by account_id {{ fivetran_utils.partition_by_source_relation(package_name='recurly') }} order by account_month) as account_month_number,
...
current_vs_previous_mrr as (
    ...
    select
        ...
        current_month_mrr,
        lag(current_month_mrr) over (partition by account_id {{ fivetran_utils.partition_by_source_relation(package_name='recurly') }} order by account_month) as previous_month_mrr
    from mrr_by_account
),
